function CStaticSymbolCell(iRow,iCol,iXPos,iYPos,oParentContainer){
    ...
    this._init = function(iRow,iCol,iXPos,iYPos){
        ...
        _oImageAttach.scale = 0.5 * REEL_SCALE;
        var oParent= this;
        _aSprites = new Array();
        ...
        _oAmountBg.x = WIN_BIG_ANIM_WIDTH/2+_oWinningFrameBig.x;
        _oAmountBg.y = WIN_BIG_ANIM_HEIGHT-oSpriteAmount.height/2+_oWinningFrameBig.y-6;
        _oImageAttach.addChild(_oAmountBg);

        
        _oAmountText = new CTLText(_oImageAttach, 
                    _oAmountBg.x-oSpriteAmount.width/2,_oAmountBg.y-oSpriteAmount.height/2, oSpriteAmount.width, oSpriteAmount.height-20, 
                    76, "center", "#dab977", FONT_GAME_1, 1.1,
                    0, 0,
                    " ",
                    true, true, false,
                    false );
        ...
        var oData = {   // image to use
                        ...
                        framerate:40,
                        // width, height & registration point of each sprite
                        frames: {width: 285, height: 264}, 
                        animations: {  static: 0,anim_0:[0,42],anim_1:[15,42,"anim_0"],anim_2:[30,42,"anim_0"] }
        };

        var oSpriteSheet = new createjs.SpriteSheet(oData);
        
        _oWinningFrame = createSprite(oSpriteSheet, "static",0,0,285,264);
        _oWinningFrame.x = iXPos-45;
        _oWinningFrame.y = iYPos-50;
        _oWinningFrame.scale = REEL_SCALE;
    ...
    this.show = function(iValue,iAmountWin,oPos,oRegPoint,iAnimCycle){
        ...
        createjs.Tween.get(_oImageAttach).to({scale:REEL_SCALE }, 1000,createjs.Ease.cubicOut);
    ...
    this.showWinFrame = function(){
        ...
        _oWinningFrame.gotoAndPlay("anim_"+iRand);
    ...
    this._onAnimEnded = function(){
        ...
        if(_iNumCycles === _iTotAnimCycle && _iCurSpriteAnimating > -1){
            ...
            createjs.Tween.get(_oImageAttach).wait(1000).to({scale:0.52 ,alpha:0}, 500,createjs.Ease.cubicOut).call(function(){_oThis.stopAnim();s_oGame.showWin();});
        }
    };
