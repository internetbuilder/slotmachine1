function CGame(){
    ...
    var _oLogo;
    var _oFrontSkin;
    var _oFrontSkinFreespins;
    var _oFreespinPanel;
    var _oFreespinResult;
    var _oInterface;
    var _oPaytableLandscape;
    var _oPaytablePortrait;
    var _oBonusPanel;
    var _oContainerSlot;
    var _oContainerReels;
    var _oContainerWinAnims;
    var _oAvatar;
    ...
    this._init = function(){
        ...
        _oBgController = new CBgController(s_oAttachSection);
        
        
        _oContainerSlot = new createjs.Container();
        _oContainerSlot.x = REEL_OFFSET_X;
        _oContainerSlot.y = REEL_OFFSET_Y;
        _oContainerSlot.scale = REEL_SCALE;
        s_oAttachSection.addChild(_oContainerSlot);
        
        
        
        _oCable = createBitmap(s_oSpriteLibrary.getSprite("bg_cable"));
        s_oAttachSection.addChild(_oCable);
        
        _oCableFreespin = createBitmap(s_oSpriteLibrary.getSprite("bg_cable_freespin"));
        _oCableFreespin.alpha = 0;
        s_oAttachSection.addChild(_oCableFreespin);
        
        this._initReels();
        
        
        _oSpriteMaskSlot = s_oSpriteLibrary.getSprite('mask_slot');
        _oFrontSkin = createBitmap(_oSpriteMaskSlot);
        _oContainerSlot.addChild(_oFrontSkin);
        
        _oFrontSkinFreespins = createBitmap(s_oSpriteLibrary.getSprite('mask_slot_freespins'));
        _oFrontSkinFreespins.alpha = 0;
        _oContainerSlot.addChild(_oFrontSkinFreespins);
        
        //ADD SUSPANCE ANIM
        _oSuspanceReel = new CSuspanceEffect(_oContainerSlot);
        
        
        _oContainerSlot.regX = _oSpriteMaskSlot.width/2 + _oFrontSkin.x;
        _oContainerSlot.regY = _oSpriteMaskSlot.height/2 + _oFrontSkin.y;
        
        var oData = {   // image to use
                        images: [s_oSpriteLibrary.getSprite('logo')], 
                        framerate:40,
                        // width, height & registration point of each sprite
                        frames: {width: 542, height: 224,regX:271,regY:0}, 
                        animations: {  normal: 0,freespin:[1,23] }
       };
       
        _pStartPosLogo = {x:_oSpriteMaskSlot.width/2,y:-70};
        var oSpriteSheet = new createjs.SpriteSheet(oData);
        _oLogo = createSprite(oSpriteSheet, "normal",271,0,542,224);
        ...
        _oAvatar = new CAvatarController(s_oAttachSection);
        _oAvatar.show("idle");

        _oPaytableLandscape = new CPaytableLandscape();
	_oPaytablePortrait = new CPaytablePortrait();
        ...
        if(isSoundPlaying("soundtrack_menu")){
            fadeStopSound("soundtrack_menu",s_aSounds["soundtrack_menu"].volume(),0,3000);
        }else{
            stopSound("soundtrack_menu");
        }
    ...
    this.unload = function(){
        ...
        _oInterface.unload();
        _oPaytableLandscape.unload();
        _oPaytablePortrait.unload();
    ...
    this.refreshButtonPos = function(){
        ...
        if(s_bLandscape){
            _oContainerSlot.y = REEL_OFFSET_Y;
            _oContainerWinAnims.y = REEL_OFFSET_Y;
            
            var iCurViewHeight = CANVAS_HEIGHT -(s_iOffsetY*2);
            if(iCurViewHeight < _oSpriteMaskSlot.height){
                REEL_SCALE = iCurViewHeight/_oSpriteMaskSlot.height
            }
            _oPaytableLandscape.refreshButtonPos();
            
            if(_bShowingPaytable && !_oPaytableLandscape.isVisible()){
                _oPaytablePortrait.hide();
                _oPaytableLandscape.show(_oPaytablePortrait.getCurPage());
            }
            _oInterface.showButPaylines();
        }else{
            _oContainerSlot.y = REEL_OFFSET_Y - 100;
            _oContainerWinAnims.y = REEL_OFFSET_Y-100;
            
            var iCurViewWidth = CANVAS_WIDTH - (s_iOffsetX*2)+340;
            if(iCurViewWidth < _oSpriteMaskSlot.width){
                REEL_SCALE = iCurViewWidth/_oSpriteMaskSlot.width;
                _oInterface.hideButPaylines();
            }else{
                _oInterface.showButPaylines();
            }
            _oPaytablePortrait.refreshButtonPos();
            
            if(_bShowingPaytable && !_oPaytablePortrait.isVisible()){
                _oPaytableLandscape.hide();
                _oPaytablePortrait.show(_oPaytableLandscape.getCurPage());
            }
        }

        
        _oContainerSlot.scale = REEL_SCALE;
        _oContainerWinAnims.scale = REEL_SCALE;
        
        var iSlotY = (_oContainerSlot.y+_oSpriteMaskSlot.height/2)+100;
       
        if(iSlotY > CANVAS_HEIGHT-s_iOffsetY){
            iSlotY = CANVAS_HEIGHT-s_iOffsetY+80;
        }
        _oAvatar.refreshPos(iSlotY);
    ...
    this._initReels = function(){  
        ...
        var iXPos = START_REEL_OFFSET_X = 223;
        var iYPos = START_REEL_OFFSET_Y = 105;
    ...
    this._initStaticSymbols = function(){
        _oContainerWinAnims = new createjs.Container();
        _oContainerWinAnims.x = _oFrontSkin.x+_oSpriteMaskSlot.width/2 + SYMBOL_WIDTH/2+30;
        _oContainerWinAnims.y = REEL_OFFSET_Y;
        s_oAttachSection.addChild(_oContainerWinAnims);
        ...
        for(var i=0;i<NUM_ROWS;i++){
            ...
            for(var j=0;j<NUM_REELS;j++){
                var oSymbol = new CStaticSymbolCell(i,j,iXPos,iYPos,_oContainerWinAnims);
        ...
        _oContainerWinAnims.regX = _oContainerWinAnims.getBounds().width/2;
        _oContainerWinAnims.regY = _oContainerWinAnims.getBounds().height/2;
        
        for(var i=0;i<NUM_ROWS;i++){
            for(var j=0;j<NUM_REELS;j++){
                _aStaticSymbols[i][j].setVisible(false);
            }
        }
    };
    ...
    this.stopNextReel = function() {
        ...
        if(_iNextColToStop >1 && (_iCurNumBonusSymbolsInReels === 2 || _iCurNumFreespinSymbolsInReels === 2)){
            ...
            _oSuspanceReel.show(132+(_iNextColToStop)*(SYMBOL_WIDTH + SPACE_BETWEEN_SYMBOLS),START_REEL_OFFSET_Y-35);
        }else{
        ...
        if(_aWinningLine.length > 0){
            ...
            for(var i=0;i<_aWinningLine.length;i++){
                if(_aWinningLine[i].value<7){
                    _oPaytableLandscape.highlightCombo(_aWinningLine[i].value,_aWinningLine[i].num_win);
                    _oPaytablePortrait.highlightCombo(_aWinningLine[i].value,_aWinningLine[i].num_win);
                }
            ...
            if(_iTotFreeSpin > 0){
                ...
                if(_oLogo.currentAnimation === "freespin"){
                    ...
                    createjs.Tween.get(_oCableFreespin).to({alpha: 0}, 1500, createjs.Ease.cubicOut);
                    createjs.Tween.get(_oFrontSkinFreespins).to({alpha: 0}, 1500, createjs.Ease.cubicOut);
                    _oAvatar.hideFreespin();
                    
                    _oBgController.stopFreespin();
    ...
    this.showWin = function(){
        ...
        _aStaticSymbols[iRow][iCol].show(_aFinalSymbolCombo[iRow][iCol]+1,_aWinningLine[_iCurWinShown].amount,oPos,oRegPoint,(_bAutoSpin || _bActivateFreespin === false && _iTotFreeSpin >0)?1:1);
    ...
    this.removeWinShowing = function(){
        _oPaytableLandscape.resetHighlightCombo();
        _oPaytablePortrait.resetHighlightCombo();
   ...
   this._generateLosingPattern = function(){	
    ...
    this.onInfoClicked = function(){
        ...
        _bShowingPaytable = true;
        if(s_bLandscape){
            if(_oPaytableLandscape.isVisible()){
                _oPaytableLandscape.hide();
            }else{
                _oPaytableLandscape.show(0);
            }
        }else{
            if(_oPaytablePortrait.isVisible()){
                _oPaytablePortrait.hide();
            }else{
                _oPaytablePortrait.show(0);
            }
        }
    };
    ...
    this.exitFromFreespinPanel = function(){
        ...
        _oBgController.startFreespin();
        
        createjs.Tween.get(_oCableFreespin).to({alpha: 1}, 1500, createjs.Ease.cubicOut);
        createjs.Tween.get(_oFrontSkinFreespins).to({alpha: 1}, 1500, createjs.Ease.cubicOut);
        _oAvatar.startFreespin();
